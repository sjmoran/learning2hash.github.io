...
- **[Rensa (beowolx) – High‑performance MinHash](https://github.com/beowolx/rensa)**: A Rust‑based MinHash implementation with Python bindings, designed for fast similarity estimation and deduplication of large datasets. Benchmarks show it's up to ~40 × faster than the popular `datasketch` library while using less memory  [oai_citation:0‡github.com](https://github.com/beowolx/rensa?utm_source=chatgpt.com).
